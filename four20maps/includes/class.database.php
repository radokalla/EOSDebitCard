...
class DB {
	...
	function update($table, $data, $id) {
		...
		$sql = "UPDATE ".$this->escape($table)." SET ".$update." WHERE ".key($id)."=".$this->escape(current($id));
	...
	function Loginr($table, $post)
	{
		$password=md5($post['password']);
		$qrg="Select * from ".$table." where username='".$post['username']."' and password='".$password."'";
		
		$result = mysql_query($qrg);
		$num_rows = mysql_num_rows($result);
		$my_fetch=mysql_fetch_assoc($result);
		if($num_rows >0)
		{
			$_SESSION["uid"]=$my_fetch['id'];
			$msgr="Succes fully Login";
		header('location:'.ROOT_URL.'register.php?msg='.$msgr.'&reg='.$my_fetch['id']);	
		}
		else
		{
