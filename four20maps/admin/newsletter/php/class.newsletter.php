...
class newsletter
{
    ...
    public function send_email($email_to, $email_subject, $email_contents, $email_from, $email_from_name, $options = array())
    {
        ...
        /*print ($email_to."<br />");
        print ($email_subject."<br />");
        print ($email_contents."<br />");
        print ($email_from."<br />");
        print ($email_from_name."<br />");
        print_r ($options."<br />");
        exit;*/
  ...
  if ($options['from'])
    ...
    public function write_newsletter_content($db, $newsletter_id)
    {
        ...
        $newsletter_html_small = $this->get_template_html($newsletter_data['template'], array() , array(
            'small' => true,
            'full' => false
        )); // pass options to template, so different full/small email template
        ...
        $newsletter_html_full = $this->get_template_html($newsletter_data['template'], array() , array(
            'small' => false,
            'full' => true
        )); // pass options to template, so different full/small email template
    ...
    public function get_group($db, $group_id)
    {
        ...
        $reference_parameter=qa($sql, $db);
        return array_shift($reference_parameter);
    ...
    public function get_bounce_members($db)
    {

        if (!$db) $db = db_connect();

        $sql = "SELECT * FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.member_id = m.member_id AND nm.bounce_time > 0";
    ...
    public function get_link($db, $id)
    {
        ...
        $sql = "SELECT * FROM `" . _DB_PREFIX . "link` WHERE link_id = '" . mysql_real_escape_string($id) . "'";
         $reference_parameter=qa($sql, $db);
        $link = array_shift($reference_parameter);
    ...
    public function get_members($db, $group_id = false, $newest_first = false, $limit = false, $search = array())
    {
        ...
        if (!isset($search['bounce_id']))
        {

            $search['bounce_id'] = array();
        ...
        if (count($search['bounce_id']))
        {

            $sql .= " LEFT JOIN `" . _DB_PREFIX . "newsletter_member` nm USING (member_id) WHERE m.member_id = nm.member_id AND nm.bounce_time > 0";
        ...
        foreach ($search['bounce_id'] as $search_bounce_id => $tf)
        {

            $sql .= " AND nm.group_id = '" . (int)$search_group_id . "' ";
    ...
    public function save_member($db = false, $member_id = 'new', $fields, $from_public_sub = false)
    {
        ...
        if ($fields['email'])
        {
            ...
            $reference_parameter=qa($sql, $db);
            $existing = array_shift($reference_parameter);
    ...
    public function delete_members($db, $deleteCB)
    {

        if (!$db) $db = db_connect();

        $count_arr = count($deleteCB);
        ...
        for ($i = 0;$i < $count_arr;$i++)
        {
            ...
            $que = "delete FROM member where member_id =" . $del_id;
            ...
            $que = "delete FROM member_group where member_id =" . $del_id;
    ...
    public function delete_bounceMembers($db)
    {
        ...
        while ($row = mysql_fetch_assoc($res))
        {
            ...
            $que = "delete FROM newsletter_member where member_id =" . $member_id;
            ...
            $que = "delete FROM member where member_id =" . $member_id;
            ...
            $que = "delete FROM member_group where member_id =" . $member_id;
    ...
    public function get_stats_send($db, $send_id)
    {
        ...
        $reference_parameter=qa($sql, $db);
        $send = array_shift($reference_parameter);

        $sql = "SELECT count( nm.`send_id` ) as unsent_members FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.send_id = '" . mysql_real_escape_string($send_id) . "' AND nm.status = 1 AND m.unsubscribe_date = '0000-00-00'";

        $arr = qa($sql, $db);

        $send['unsent_members'] = $arr[0]['unsent_members'];

        $sql = "SELECT   count( nm.`send_id` ) as sent_members FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.send_id = '" . mysql_real_escape_string($send_id) . "' AND nm.status != 1";

        $arr = qa($sql, $db);
        $send['sent_members1'] = $arr[0]['sent_members'];

        $sql = "SELECT  * FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.send_id = '" . mysql_real_escape_string($send_id) . "' AND nm.status != 1 limit 0,3000";

        $arr = qa($sql, $db);
        $send['sent_members'] = $arr[0]['sent_members'];

        $sql = "SELECT  count( nm.`send_id` ) as unsub_members FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.send_id = '" . mysql_real_escape_string($send_id) . "' AND m.unsubscribe_date != '0000-00-00' AND m.unsubscribe_send_id = '" . mysql_real_escape_string($send_id) . "'";

        $arr = qa($sql, $db);
        $send['unsub_members'] = $arr[0]['unsub_members'];

        $sql = "SELECT  count( nm.`send_id` ) as opened_members FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.send_id = '" . mysql_real_escape_string($send_id) . "' AND nm.open_time > 0";

        $arr = qa($sql, $db);
        $send['opened_members'] = $arr[0]['opened_members'];

        $sql = "SELECT  count( nm.`send_id` ) as bounce_members FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.send_id = '" . mysql_real_escape_string($send_id) . "' AND nm.bounce_time > 0";

        $arr = qa($sql, $db);
        $send['bounce_members'] = $arr[0]['bounce_members'];
        //echo "rajesh";exit;
        return $send;
    ...
    public function get_pending_sends($db, $newsletter_id = false)
    {
        ...
        foreach ($sends as & $send)
        {
            ...
            $reference_parameter=qa($sql, $db);
            $unsent = array_shift($reference_parameter);
    ...
    public function send_out_newsletter($db = false, $send_id, $member_id, $newsletter_id, $newsletter_data, $force = false)
    {
        ...
        if ($status)
        {
            ...
            if (!$force && $res_r['status'] != '1') return false;
            ...
            $newsletter_html = $res_r['template_html'];
            ...
            foreach ($replace as $key => $val)
            {
                ...
                $newsletter_html = str_replace('{' . strtoupper(preg_quote($key, '/')) . '}', $val, $newsletter_html);

            }

            /*print ($replace['email_subject']."<br />");
            print ($replace['from_name']."<br />");
            print ($replace['from_email']."<br />");
            print ($replace['to_email']."<br />");
            print ($replace['member_id']."<br />");
            print ($replace['send_id']."<br />");
            print ($replace['email']."<br />");
            exit;*/
    ...
    public function is_email_limit_ok($db)
    {
        ...
        foreach ($this->settings as $key => $val)
        {
            ...
            if ($start_time && $send_limit)
            {
                ...
                $reference_parameter=qa($sql, $db);
                $res = array_shift($reference_parameter);
    ...
    public function get_campaign($db = false, $campaign_id)
    {
        ...
        $reference_parameter=qa($sql, $db);
        $campaign = array_shift($reference_parameter);
    ...
    public function get_sync($db = false, $sync_id)
    {
        ...
        $reference_parameter=qa($sql, $db);
        $sync = array_shift($reference_parameter);
...
?>
							...
							<tr>
								...
								<td><input type="checkbox" name="group_id[]" value="<?php echo $group['group_id']; ?>" <?php if (!$member_id || $member_data['groups'][$group['group_id']]) echo 'checked'; ?>></td>
...
?>
							...
							<tr>
								...
								<td><input type="checkbox" name="campaign_id[]" value="<?php echo $campaign['campaign_id']; ?>" <?php if (!$member_id || $member_data['campaigns'][$campaign['campaign_id']]) echo ' checked'; ?>></td>
