<script type="text/javascript" src="layout/js/checkbox.js"></script>
<script type="text/javascript" src="layout/js/jquery-ui.min.js"></script>
<link href="layout/css/jquery-ui-1.8.7.custom.css" type="text/css" rel="stylesheet" />
...
if($_REQUEST['deleteAll']){
	extract($_REQUEST);
	//echo "<pre>";print_r($deleteCB);exit;
	$newsletter->delete_members($db,$deleteCB);
	ob_end_clean();
...
if($_REQUEST['delBounce']){
	$newsletter->delete_bounceMembers($db);
	ob_end_clean();
	header("Location: index.php?p=members");
	exit;
}
...
<?php
	...
	$bounce = $newsletter->get_bounce_members($db);
...
<div class="box">
	...
	<table cellpadding="5">
		<tr>
			...
			<td valign="middle">
				<input type="text" name="search_fields[email]" id="email" value="<?php echo htmlspecialchars($search['email']);?>" size="12">
			</td>
			<td valign="middle">
				Name:
			</td>
			<td valign="middle">
				<input type="text" name="search_fields[name]" id="namesearch" value="<?php echo htmlspecialchars($search['name']);?>" size="12">
			</td>
			<td valign="middle">
				Groups:
			</td>
			...
			<td valign="middle">
            	<input type="checkbox" name="search_fields[bounce_id]" <?php echo (isset($search['bounce_id'])) ? 'checked' : '';?> value="true"> Select Bounce Members
            </td>
			<td valign="middle">Un-Subscribe Members</td>
            <td valign="middle"></td>
...
<? if( isset($search['bounce_id']) ){ ?>
    <h2><span>Delete All Bounced Members</span></h2>
    <div class="box">
        <table cellpadding="5">
            <tr>
                <td align="center" style="text-align:center;">
                    <a href="?p=members&delBounce=true" onclick="if(confirm('Really delete All the Bounced Members?'))return true;else return false;">Delete All Bounce Members</a>
                </td>
            </tr>
        </table>
    </div>
...
<div class="box">
	...
	?>
			...
			<tr>
				<th>
                <input type="checkbox" name="deleteCell[]" id="selectAll" />
                </th>
                <th>Email Address</th>
				<th>First Name</th>
				<th>Last Name</th>
            ...
            <tr id="del_tr" style="display:none; width:100%;">
            	<td colspan="13">
                	<input type="submit" id="deleteAll" name="deleteAll" value="Delete All">
		...
		while( $x++ < $per_page_limit && $member = mysql_fetch_assoc($pagable_members)){
            ...
            <?php if( isset($search['bounce_id']) || count($member['bounces'])>0 ) {?>
			<tr style="background-color:#FFF0F0;">
			<? }else{?>            
            <tr>
            <? }?>
				<td><input type="checkbox" id="deleteCB[]" name="deleteCB[]" value='<?=$member['member_id']?>' /></td>
                <td>
	...
	<input type="submit" name="download_as_csv" value="Download above as CSV">
...
<div class="box">
	<a href="?p=members&export=true">Export all members to CSV</a>
</div>
