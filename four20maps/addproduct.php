...
if(isset($_GET['prodid'])) {
	...
	$cst = mysql_query("select productID as id,price,ten_price,hundered_price,qty,productDescription,weight from Products where categoryID =".$_GET['prodid']);
...
?>
        ...
        <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
            ...
            <input type="hidden" name="pid" id="pid" value="<?php echo isset($cost['id'])?$cost['id']:'0';?>">
            <?php  
				$product_store_result = mysql_query("SELECT store_id FROM `store_product_map` where user_id = '".$_SESSION['StoreID']."'");
				$product_store_array=array();
				 while($product_store_result_array = mysql_fetch_array($product_store_result)) { 
					$product_store_array[]= $product_store_result_array['store_id'];
				 }
				...
				if(mysql_num_rows($stores_result) == 0)
				{
					header("location:".ROOT_URL."stores.php");
				}
				
				?>
  				<div class="form-group">
					<label class="col-lg-3 col-md-3 col-sm-3 col-xs-12 control-label">Store Name<span class="require">*</span></label>
					<div class="col-lg-8 col-md-8 col-sm-9 col-xs-12">
						<select class="form-control required" id="store_name" name="store_name[]" title="Store Name" multiple>
							<?php while($store_array = mysql_fetch_array($stores_result)) { ?>
							<option value="<?=$store_array["id"];?>" <?php echo (isset($product_store_array)&& (in_array($store_array["id"],$product_store_array) || mysql_num_rows($stores_result))) ? 'selected' : '';?>><?=$store_array["name"];?></option>
							<?php } ?>
                ...
                <div class="col-lg-8 col-md-8 col-sm-9 col-xs-12">
                      ...
                      <div class="form-group">
                <label class="col-lg-3 col-md-3 col-sm-3 col-xs-12 control-label">Product Alias Name<span class="require">*</span></label>
                <div class="col-lg-8 col-md-8 col-sm-9 col-xs-12">
                        <input type="text" id="categoryAliasName" name="categoryAliasName"  maxlength="100"
						value="<?php echo isset($prodcatDet['categoryAliasName'])?$prodcatDet['categoryAliasName']:'';?>" placeholder="Product Alias Name" 
						class="form-control required alpha">
                      </div>
              </div>
                    <div class="form-group">
			 ...
			 foreach($return_options as $optionID=>$optionDetails) {?>
			  ...
			  ?>
			   ...
			   <div class="form-group">
                   ...
                   <div class="form-group" <?=($_SESSION["SubscriptionCategoryId"] != "4") ? 'style="display:none"' : '';?>>
					<label class="col-lg-3 col-md-3 col-sm-3 col-xs-12 control-label">  Price for 10 <span class="require"> *</span></label>
					<div class="col-lg-8 col-md-8 col-sm-9 col-xs-12">
						<input type="text" class="form-control" name="ten_price" <?php if(isset($_GET['prodid'])){ ?> value="<?php echo $cost['ten_price'];} ?>" placeholder="Enter the price of the product" id="ten_price" Onblur="cost_valid(this.id)">
					</div>
				</div>
                   <div class="form-group"  <?=($_SESSION["SubscriptionCategoryId"] != "4") ? 'style="display:none"' : '';?>>
					<label class="col-lg-3 col-md-3 col-sm-3 col-xs-12 control-label">Price for 100 <span class="require"> *</span></label>
					<div class="col-lg-8 col-md-8 col-sm-9 col-xs-12">
						<input type="text" class="form-control" name="hundered_price" <?php if(isset($_GET['prodid'])){ ?> value="<?php echo $cost['hundered_price'];} ?>" placeholder="Enter the price of the product" id="hundered_price" Onblur="cost_valid(this.id)">
					</div>
				</div>
                    <div class="form-group"  <?=($_SESSION["SubscriptionCategoryId"] != "4") ? 'style="display:none"' : '';?>>
					<label class="col-lg-3 col-md-3 col-sm-3 col-xs-12 control-label">Weight  (in Lbs)<span class="require"> *</span></label>
					<div class="col-lg-8 col-md-8 col-sm-9 col-xs-12">
						<input type="text" class="form-control" name="weight" <?php if(isset($_GET['prodid'])){ ?> value="<?php echo $cost['weight'];} ?>" placeholder="Enter the Weight of the product" id="weight">
					</div>
				</div>  
                   <div class="form-group">
