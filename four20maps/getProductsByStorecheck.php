  ...
  $res = mysql_query("call map_proc_getProductsByStore('".$_REQUEST["q"]."','".$_REQUEST["keyword"]."');") or die(mysql_error());
	...
	if(mysql_num_rows($res) > 0)
	{
		...
		//echo '<pre>'; print_r($result_arr);
			echo json_encode($result_arr,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);	 exit; 
			 ...
			 if($row['categoryID'] == $cat_id)
			 {
				...
				/* if($row['productID'] == "116"){
				 ...
				 $productsForCat[$row['productID']] = array(
						...
						'productDescription' => (isset($row['productDescription']) && !empty($row['productDescription'])) ? trim(iconv('UTF-8', 'UTF-8//IGNORE', $row['productDescription'])) : "Description not found.",
						'price'=>$row['price'],
						'image'=>$row['image'],
